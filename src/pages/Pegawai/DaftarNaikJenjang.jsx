...
function DaftarNaikJenjang() {
  const [dataUsulan, setDataUsulan] = useState(null);
  const history = useHistory();
  async function fetchUsulan() {
    axios
      .get(
        `${import.meta.env.VITE_REACT_APP_API_BASE_URL}/naik-jenjang/get/usulan`
      )
      .then((res) => {
                ...
                <Tr key={index}>
                  ...
                  <Td>
                    <Button
                      onClick={() =>
                        history.push(
                          `/pegawai/detail-naik-jenjang/${item.pegawai.id}`
                        )
                      }
                    >
                      Detail
...
export default DaftarNaikJenjang;
